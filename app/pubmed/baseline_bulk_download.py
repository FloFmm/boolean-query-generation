...
for xml_file in xml_files:
    ...
    while attempt < 3:
        try:
            ...
            # Decompress XML
            with (
                gzip.open(local_gz_path, "rb") as f_in,
                open(local_xml_path, "wb") as f_out,
            ):
    ...
    with (
        open(local_xml_path, "r", encoding="utf-8") as f_in,
        open(local_jsonl_path, "w", encoding="utf-8") as f_out,
    ):
        tree = ET.parse(f_in)
        root = tree.getroot()
