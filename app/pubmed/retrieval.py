...
def search_pubmed_dynamic(query, start_year=1800, end_year=2025, target_count=9500, max_retrieved=100_000):
    ...
    if total_expected > max_retrieved:
        raise optuna.exceptions.TrialPruned(f"Tried to retrieve more than {max_retrieved} PubMed documents")
    ...
    while current_end >= start_date:
        current_start = max(
            current_end - datetime.timedelta(days=int(window_days)), start_date
        )
        ...
        if DEBUG:
            print(
                f"{mindate}-{maxdate}: {count_pmids} results (window {int(window_days)} days)"
            )
        ...
        if remaining < 9999:
            record = search_pubmed_date_range(
                query, start_date.strftime("%Y/%m/%d"), current_end.strftime("%Y/%m/%d")
            )
            pmids = record["IdList"]
            count_pmids = record["Count"]
            if DEBUG:
                print(
                    f"{start_date.strftime('%Y/%m/%d')}-{current_end.strftime('%Y/%m/%d')}: {count_pmids} results"
                )
            all_pmids.update(pmids)
            break
...
def fetch_pubmed_records(id_list):
    ...
    for start in range(0, len(id_list), BATCH_SIZE):
        ...
        handle = Entrez.efetch(
            db="pubmed", id=batch_ids, rettype="medline", retmode="text"
        )
...
def get_random_pubmed_documents(search_term, n=9999):
    ...
    """
    id_list = search_pubmed(search_term, retmax=1_000_000)[
        "IdList"
    ]  # return 9999 ids at most (capped by pubmed)
...
def sample_jsonl_files(folder_path, n_files, n_lines):
    ...
    for file_path in sampled_files:
        ...
        chosen_indices = set(
            random.sample(range(total_lines), min(n_lines * 3, total_lines))
        )
            ...
            for i, line in enumerate(f):
                ...
                if i in chosen_indices:
                    try:
                        ...
                        if all(
                            k in data and data[k] for k in ("title", "abstract", "pmid")
                        ):
...
def search_pubmed_med_cpt(
    ...
    with torch.no_grad():
        ...
        query_emb = (
            torch.nn.functional.normalize(query_emb, p=2, dim=1).squeeze(0).cpu()
        )
...
def search_pubmed_med_cpt_graph(
    ...
    if reduce_to_2d:
        reducer = TSNE(
            n_components=2, random_state=42, perplexity=min(30, len(emb_matrix) - 1)
        )
    ...
    for pmid, sim in zip(selected_pmids, selected_sims):
        edges.append({"source": "QUERY", "target": pmid, "weight": sim})
    ...
    for file_path in tqdm(all_files, desc="Processing files", unit="file"):
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            for line in f:
                try:
                    data = json.loads(line)
                    pmid, abstract, title, mesh_terms, bag_of_words = (
                        data["pmid"],
                        data["abstract"],
                        data["title"],
                        data["mesh_terms"],
