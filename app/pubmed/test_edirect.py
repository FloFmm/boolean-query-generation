...
def fetch_all_pmids(query, output_file="pmids.txt", batch_size=100000):
    ...
    esearch_out = subprocess.run(esearch_cmd, shell=True, capture_output=True, text=True)
    print(len(esearch_out.stdout.split('\n')))

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Fetch all PMIDs for a PubMed query using EDirect")
    parser.add_argument("query", help="PubMed query string, e.g. 'cancer'")
    parser.add_argument("-o", "--output", default="pmids.txt", help="Output file (default: pmids.txt)")
    parser.add_argument("-b", "--batch", type=int, default=10000, help="Batch size per efetch (default: 100000)")
