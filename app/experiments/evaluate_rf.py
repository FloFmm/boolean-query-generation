...
from app.dataset.utils import generate_labels_and_sample_weights, rf_statistics_path, qg_statistics_path, load_synonym_map
...
def evalaute_rf(query_id, X, positives, feature_names, sorted_ids, ordered_pmids, rf_params, qg_params, term_expansions=None):
    dataset, _, end_year = review_id_to_dataset(query_id)
    
    rf_base_path = rf_statistics_path(**rf_params)
    ...
    qg_base_path = qg_statistics_path(rf_args=rf_params, qg_args=qg_params)
    ...
    pseudo_labels, sample_weight, top_k = generate_labels_and_sample_weights(k=rf_params["top_k"],
    ...
    if rf_model_path.exists():
        ...
        st=time.time()
        rf.fit(
            X, np.array(pseudo_labels), feature_names=feature_names, sample_weight=sample_weight
        )
