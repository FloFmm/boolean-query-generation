...
model = AutoModelForCausalLM.from_pretrained(model_name, cache_dir=CUSTOM_HF_PATH).to(
    device
)
...
def get_autobool_query(topic):
    ...
    messages = [
        {
            "role": "system",
            "content": "You are an expert systematic review information specialist. You are tasked to formulate a systematic review Boolean query in response to a research topic. The final Boolean query must be enclosed within <answer> </answer> tags. Do not include any explanation or reasoning.",
        },
        {
            "role": "user",
    ...
    outputs = model.generate(
        **inputs, max_length=4096, do_sample=True, temperature=0.6, top_p=0.95
    )
    ...
    for i, m in enumerate(matches):
        print(
            f"Match {i}: '{m[:15] + '...' + m[-15:] if len(m) > 30 else m}'", flush=True
        )
        print()
    query = None
    if matches:
        ...
        valid_matches = [
            m for m in matches if re.search(r"\s(or|and)\s", m, re.IGNORECASE)
        ]
        if valid_matches:
...
if __name__ == "__main__":
    ...
    if os.path.exists(output_file):
        ...
        print(
            f"Found {len(existing_query_ids)} already processed queries, skipping them..."
        )

    # Sort review_ids so priority ones come first
    sorted_review_ids = sorted(
        dataset_details.keys(), key=lambda x: (x not in priority_query_ids, x)
    )
    ...
    with open(output_file, "a") as f:
        for review_id in sorted_review_ids:
            ...
            for trial in range(max_trials):
                ...
                if query is None:
                    print(
                        f"Trial {trial + 1}/{max_trials} failed for {review_id}, query is None. Retrying..."
                    )
                    continue
                ...
                if not check_logic(query):
                    print(
                        f"Trial {trial + 1}/{max_trials} failed for {review_id}, query has invalid logic: {query}"
                    )
                    query = None
                    continue
                ...
                try:
                    ...
                    print(
                        f"Trial {trial + 1}/{max_trials} failed for {review_id}, evaluation error: {e}. Retrying..."
                    )
            ...
            if query is None or precision is None:
                raise ValueError(
                    f"Failed to generate and evaluate query for {review_id} after {max_trials} attempts"
                )
            ...
            print(
                f"Completed {review_id}: Precision={precision:.4f}, Recall={recall:.4f}"
            )
